...
export default function ChatPage() {
    ...
    const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > 1024 * 1024) {
            alert('File too large. Max 1MB allowed for text processing.');
            return;
        }
    ...
    return (
        <div className="flex h-screen bg-surface-900 overflow-hidden relative">
            ...
            <main className="flex-1 flex flex-col w-full">
                ...
                {error && (
                    <div className="px-6">
                        <div className="max-w-3xl mx-auto p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm mb-4">
                            {error.message || 'An error occurred. Please check your API key in Settings.'}
                        </div>
                    </div>
                )}
